	...
	<view class="content">
		<view class="board">
			...
			<view class="wrap">
				...
				<view class="line flex flex_start">
					<view class="tit">保险批次号</view>
					<view class="text">{{batch.code}}</view>
				</view>
				<view class="line flex flex_start">
					<view class="tit">购买人数</view>
					<view class="text">{{batch.detail_count}}</view>
				</view>
				<view class="line flex flex_start">
					<view class="tit">保险单价</view>
					<view class="text">{{batch.price}}</view>
				</view>
				<view class="line flex flex_start">
					<view class="tit">保费总额</view>
					<view class="text">{{batch.amount}}</view>
				</view>
			</view>
			<!-- 雇工列表 -->
			<uni-table stripe emptyText="暂无更多数据">
				<uni-tr>
					...
					<uni-th align="left">保费(元)</uni-th>
					<uni-th align="left">保障日期</uni-th>
				...
				<uni-tr v-for="(item,index) in list" :key="index">
					...
					<uni-td>{{item.worker_hired_time}}</uni-td>
				</uni-tr>
			</uni-table>
			...
			<view class="btns flex flex_btween" v-if="(manager_type =='project_manager' && batch.status == 'project_manager_reviewing') || (manager_type =='financial_manager' && batch.status == 'finance_manager_reviewing')">
				<view class="btn refuse" @click="bindAgree('rejected')">拒绝</view>
				<view class="btn pass" @click="bindAgree('approved')">通过</view>
			</view>
			<view class="btns tips" v-if="batch.status == 'project_manager_rejected'">该审批信息已拒绝</view>
			...
			<view class="btns tips" v-if="manager_type =='financial_manager' && batch.status == 'approved'">该审批信息您已通过</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			getInfo(){
				let url = "/wechatweb/insurance-batch-review/detail?manager_type="+this.manager_type+"&token="+this.token+"&page="+this.currentPage
			...
			bindAgree(type){
				...
				let url = "/wechatweb/insurance-batch-review/review"

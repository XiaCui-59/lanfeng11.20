			...
			<view class="box" :style="{top:marginTop+tabMargin+'px'}">
				<view class="wrap">
					...
					<u--input placeholder="请输入充值金额" size="large" prefixIcon="rmb"
						prefixIconStyle="font-size: 22px;color: #303030;font-weight:900;" border="none" v-model="mount"
						fontSize="38rpx" @change="changeInput"></u--input>
	...
	export default {
		...
		methods: {
			changeInput(value) {
				...
				let result = this.priceList.filter(el => {
			...
			getOrderId() {
				...
				return new Promise(resolve => {
					// 获取订单数据接口
					let url = "/broker/company-recharges"
					_this.$request(url, {
						"amount": this.mount,

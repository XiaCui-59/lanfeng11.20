...
<script>
	...
	export default {
		...
		methods: {
			...
			getList() {
				this.$request("/broker/projects/all").then(res => {
					if (res.code == 0) {
			...
			submit() {
				...
				uni.uploadFile({
					url: app.globalData.baseUrl + '/broker/attendance-record/parse', //仅为示例，非真实的接口地址
			...
			confirm() {
				...
				let _this = this
				let url = "/broker/attendance-record-parse-result/" + this.resultData.result_id + "/confirm"

	...
	<view class="content">
		<view class="right">
			<view class="header flex flex-start">
				...
				<text style="font-size: 22px;">灵动优聘企业后台管理系统</text>
			...
			<view class="tabs flex flex_start">
				<view class="tab" :class="currentTab==1?'active':''" @click="changeTab(1)">验证码登录/注册</view>
			...
			<view class="dlbutton" hover-class="dlbutton-hover" @tap="bindLogin">
				<text>{{currentTab==1?"登录/注册":"登录"}}</text>
			</view>
			<view class="tips" v-if="currentTab==1" style="font-size: 14px;color:#f00;">注：若无账号，通过手机号验证码登录后将自动创建账号。
			</view>
		</view>
		<!-- <view class="mask">
			<view class="inner flex flex-start">
					<view class="left">
					<image src="/static/login_bg@2x.png" mode="widthFix"></image>
				</view>
				<view class="right">
					<view class="header">
						Hello !
					</view>
					<view style="margin-top: 20upx;font-size: 25px;">欢迎来到灵动优聘企业管理系统</view>
					<view class="tabs flex flex_start">
						<view class="tab" :class="currentTab==0?'active':''" @click="changeTab(0)">账号密码</view>
					...
					<view class="list">
						...
						<view class="list-call" style="margin-top: 20upx;" v-if="currentTab==0">
							<image class="img" src="/static/shilu-login/block.png"></image>
							<input class="biaoti" v-model="password" type="text" placeholder="请输入密码" password="true"
								@confirm="bindLogin" />
						</view>
						<view class="list-call" style="margin-top: 20upx;" v-if="currentTab==1">
							<image class="img" src="/static/shilu-login/veri.png"></image>
							<input class="biaoti" v-model="verCode" type="text" placeholder="请输入短信验证码"
								@confirm="bindLogin" />
							<view class="get_code" :class="canSend?'':'disabled'" @click="getMessageCode">{{tipsText}}
							</view>
						</view>
					...
					<view class="tips">没账号？<text class="mark" @click="toRegister">点这里注册</text></view>
				</view>
		...
		onShow() {
			...
			if (!token) {
				...
				this.$request("/broker/info").then(res => {
					if (res.code == 0) {
						uni.navigateTo({
							url: "/pages/index/index?page_name=home"
						})
					}
				})
			}

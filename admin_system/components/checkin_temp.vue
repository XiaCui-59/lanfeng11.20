			...
			<view class="table_wrap">
				...
				<uni-table border stripe emptyText="暂无更多数据">
					...
					<uni-tr v-for="(item,index) in list" :key="index">
						...
						<uni-td>{{item.start_line}}</uni-td>
						<uni-td>{{item.worker_name_row}}</uni-td>
						<uni-td>{{item.id_card_row}}</uni-td>
						<uni-td>{{item.start_time_row}}</uni-td>
						<uni-td>{{item.end_time_row}}</uni-td>
						<uni-td>{{item.working_time_row}}</uni-td>
						<uni-td>{{item.working_type_row}}</uni-td>
	...
	export default {
		...
		methods: {
			...
			deleteTemp(item) {
				...
				uni.showModal({
					title: "提示",
					content: "是否作废考勤模板：" + item.name + "？作废后不可恢复，请谨慎操作。",
					confirmText: "确定删除",
					confirmColor: "#f00",
					success(resp) {
						if (resp.confirm) {

...
<script>
	...
	export default {
		...
		methods: {
			...
			viewRecord(item) {
				let url = "/admin/attendance-record/" + item.id + "/details"
			...
			getList() {
				...
				let url = "/admin/attendance-records" + (!this.currentTab.value ? "" : ("/" + this.currentTab.value)) +
			...
			merge() {
				...
				if (proArr.length > 1) {
					...
					if (this.selectedIds.length < 1) {
						...
						this.$request("/admin/attendance-record/merge", data, "POST").then(res => {
							if (res.code == 0) {
								this.showMask = true
			...
			confirm() {
				...
				this.$request("/admin/generate-project-bill", data, "POST").then(res => {
					if (res.code == 0) {
						uni.showModal({
							title: "操作成功",
							content: "已成功生成账单，可前往账单管理-费用清单确认。",
							success(resp) {
								if (resp.confirm) {
									_this.setPageName({
										pageName: "cost_bill"
				...
				uni.showModal({
					...
					success(resp) {
						if (resp.confirm) {
							...
							_this.$request("/admin/attendance-record/abandoned", data, "POST").then(res => {
								if (res.code == 0) {
									uni.showToast({
										title: "作废成功",
										duration: 2000,

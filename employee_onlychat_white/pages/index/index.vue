<template>
	<view class="ai_wrap" @touchstart="bodyClick">
		...
		<view class="ai_chat" v-if="!ifSingle">
			<!-- 自定义导航 -->
			<u-navbar @rightClick="rightClick" :autoBack="true" bgColor="rgba(239, 240, 245,1)"
				titleStyle="color:#010101;font-size: 31rpx;" :height="menuButtonInfo.height+12">
				<view class="u-nav-slot" slot="left" @click="openMenu">
					<image :src="imgUrl+'/worker/ic_menu_blue.png'" mode="widthFix"
						style="width: 58rpx;vertical-align: middle;margin-right: 20rpx;background: #fff;border-radius: 50%;">
					</image>
					<text style="color:#010101;font-size: 31rpx;text-align:left;">灵动优聘找工作就两三句话</text>

				</view>
			</u-navbar>
			<!-- 聊天框 -->
			<!-- <scroll-view class="chat_wrap" :scroll-into-view="scrollView"
				:style="{height:contentHeight+'px',marginTop:marginTop+tabMargin+'px'}" :scroll-y="true" id="chatwrap"
				@scrolltoupper="getMoreHistory"> -->
			<scroll-view class="chat_wrap" :scroll-into-view="scrollView"
				:style="{height:contentHeight+'px',bottom:inputHeight?inputHeight+subTabHeight+2*tabMargin+'px':subTabHeight+50+2*tabMargin+'px',position:'fixed',}"
				:scroll-y="true" id="chatwrap" @scrolltoupper="getMoreHistory">
				<!-- 历史记录 -->
				<view class="history list">
					<!-- <view class="history list" :style="{paddingTop:menuButtonInfo.bottom+12+'px',boxSizing:'border-box'}"> -->
							...
							<view class="inline_box">
								...
								<view class="cont" :class="item.msg_type == 'text'?'textcont':''">
									...
									<rich-text :nodes="item.content"
										...
										:style="{background:item.origin == 'customer'?'#216FF9':''}"
							...
							<view class="inline_box">
								...
								<view class="cont" :class="item.msg_type == 'text'?'textcont':''">
									...
									<rich-text :nodes="item.content"
										...
										v-if="item.msg_type == 'text'"></rich-text>
									<!-- <text
										v-if="item.msg_type == 'text' && item.origin == 'customer'">{{item.content.replace(/\\n/g,"\n")}}</text> -->
									<view class="voice_line flex flex_end">
										<view class="trans" v-show="item.showTranIcon"
			...
			<view class="ques_wrap" :style="{bottom:inputHeight?inputHeight+10+'px':2*tabMargin+'px'}"
				...
				<view class="input flex flex_btween" v-show="!showRecord">
					<image :src="imgUrl+'/worker/ic_voice_blue.png'" @tap="toVoice">
					</image>
					<view class="input_right flex flex_btween">
						...
						<image :src="imgUrl+'/worker/ic_send_bgwhite.png'" mode="widthFix" @tap="sendQuestion"></image>
					</view>
				</view>
				<view class="input flex flex_btween" v-show="showRecord"
					:style="{height:subTabHeight+'px',lineHeight:subTabHeight+'px'}">
					<image :src="imgUrl+'/worker/ic_keyboard_blue.png'" @click="toInput">
					</image>
				...
				<!-- 快速按钮 -->
				<view class="btns_wrap flex flex-start" v-if="!inputHeight">
					...
					<view class="btn_item" @click="navigate('/pages/invite/invite')">邀请有礼</view>
				</view>
			...
			<view class="inner" @touchstart="maskStart" @touchend="maskEnd" @touchmove="handleMaskMove">
				...
				<u-navbar title="个人中心" @rightClick="rightClick" :autoBack="true"
					...
					<view class="u-nav-slot" slot="left" @click="closeMenu">
						...
						<!-- <image :src="imgUrl+'/worker/ic_back_blank.png'" mode="widthFix"
				...
				<view class="user_base_out">
					<view class="user_base"
						:style="{marginTop:marginTop+4*tabMargin+'px',backgroundImage:userInfo.is_vip?`url(${imgUrl}/worker/vip_bg.png)`:`url(${imgUrl}/worker/not_vip_bg.png)`,border: userInfo.is_vip?'1px solid #ffa73a':'none'}"
						@click="navigate('/pages/vip/vip')">
						<view class="user_box" :style="{paddingBottom:userInfo.is_vip?'40rpx':'auto'}">
							<image
								:src="userInfo.is_vip?imgUrl+'/worker/vip_title2.png':imgUrl+'/worker/not_vip_title.png'"
								mode="widthFix"></image>
						...
						<view class="tips flex flex-start" v-if="!userInfo.is_vip">
							<image :src="imgUrl+'/worker/ic_th.png'" mode="widthFix"></image>
							<view class="tip">限时特惠 9.9 送一年会员权益</view>
						</view>
					</view>
					<view class="vip_icon" :style="{bottom:userInfo.is_vip?'0rpx':'16rpx'}">
						<image
							:src="userInfo.is_vip?imgUrl+'/worker/vip_icon_short.png':imgUrl+'/worker/not_vip_icon_short.png'"
							mode="widthFix"></image>
					</view>
				</view>

				<!-- <view class="user_base" :style="{marginTop:marginTop+tabMargin+'px'}"
					@click="navigate('/pages/vip/vip')">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getContentHeight() {
				// 示例中假设获取内容高度的逻辑
				// let bottom = this.inputHeight ? this.inputHeight + 10 : 2 * this.tabMargin
				// this.contentHeight = app.globalData.systemHeight - this.menuButtonInfo.top - bottom
				this.contentHeight = app.globalData.systemHeight - 5 * this.tabMargin - this.subTabHeight - this
					.marginTop - 30; // 假设内容高度是1000px
			},
			showTrans(item, type) {
